    ...
    public Mono<UploadResponse> uploadVideo(String videoId, String appId, FilePart filePart) {
        ...
        return validateFile(filePart)
                .flatMap(valid -> {
                    ...
                    Sinks.Many<UploadProgressEvent> progressSink = Sinks.many().multicast().onBackpressureBuffer();
                    progressSinks.put(videoId, progressSink);

                    // Upload to MinIO
                    return storageService.uploadFile(videoId, appId, filePart, progressSink)
                            .flatMap(objectKey -> {
                                String storageBasePath = storageService.getStorageBasePath(appId, videoId);

                                // Update catalog service
                                UpdateAfterUploadRequest catalogUpdate = UpdateAfterUploadRequest.builder()
                                        .storageBasePath(storageBasePath)
                                        .sourceFileName(filePart.filename())
                                        .sourceFileSizeBytes(0L) // Will be updated from actual file size
                                        .sourceContentType(filePart.headers().getContentType() != null
                                                ? filePart.headers().getContentType().toString()
                                                : "application/octet-stream")
                                        .status("UPLOADED")
                                        .build();

                                return catalogService.updateAfterUpload(videoId, catalogUpdate)
                                        .then(Mono.just(objectKey))
                                        .then(Mono.fromCallable(() -> {
                                            // Publish transcode message
                                            TranscodeMessage transcodeMessage = TranscodeMessage.builder()
                                                    .videoId(videoId)
                                                    .appId(appId)
                                                    .sourceFilePath(objectKey)
                                                    .storageBasePath(storageBasePath)
                                                    .fileName(filePart.filename())
                                                    .contentType(catalogUpdate.getSourceContentType())
                                                    .build();

                                            return transcodeMessage;
                                        }));
                            })
                            .flatMap(transcodeMessage -> messagePublisher.publishTranscodeMessage(transcodeMessage)
                                    .then(Mono.just(transcodeMessage)))
                            .map(transcodeMessage -> UploadResponse.builder()
                                    .videoId(videoId)
                                    .message("File uploaded successfully and queued for transcoding")
                                    .fileName(filePart.filename())
                                    .fileSize(0L)
                                    .storageBasePath(transcodeMessage.getStorageBasePath())
                                    .status("UPLOADED")
                                    .build())
                            .doOnSuccess(response -> log.info("Upload completed successfully for videoId: {}", videoId))
                            .doOnError(error -> {
                                log.error("Upload failed for videoId: {}", videoId, error);
                                // Emit failure event
                            ...
                            .doFinally(signalType -> {
                                // Complete and remove progress sink
                                Sinks.Many<UploadProgressEvent> sink = progressSinks.remove(videoId);
                                if (sink != null) {
                                    sink.tryEmitComplete();
                                }
                            });
    ...
    private Mono<Boolean> validateFile(FilePart filePart) {
        ...
        if (!validExtension) {
            return Mono.error(new IllegalArgumentException(
                    "Invalid file type. Allowed extensions: " + String.join(", ", ALLOWED_EXTENSIONS)));
        }
